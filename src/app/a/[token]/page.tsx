...
function AdminDashboardContent() {
  ...
  const copyQrToClipboard = async () => {
    try {
      ...
      // Helper function to convert data URL directly to blob (works better on iOS)
      ...
      // iOS Safari requires ClipboardItem API
      if (navigator.clipboard && navigator.clipboard.write && typeof ClipboardItem !== 'undefined') {
        try {
          let blob: Blob
          
          // For data URLs on iOS, convert directly to blob (more reliable)
          if (qrCodeUrl.startsWith('data:image')) {
            try {
              blob = dataURLToBlob(qrCodeUrl)
              // Ensure it's PNG type
              if (!blob.type || blob.type !== 'image/png') {
                blob = new Blob([blob], { type: 'image/png' })
              }
            } catch (dataUrlError) {
              // Fallback to canvas method if direct conversion fails
          ...
          // Ensure blob has correct type for iOS
          ...
          console.log('Primary clipboard method failed, trying alternative:', clipboardError)
          
          // Alternative: Try canvas method if direct conversion failed
          if (!copyCompleted) {
            try {
      ...
      // Fallback: Try legacy method (usually doesn't work on iOS but worth trying)
      if (!copyCompleted) {
        try {
          ...
          {qrCodeUrl && (
            <div className="glass-card rounded-2xl p-6 sm:p-8 text-white">
              ...
              <div className="flex flex-col lg:flex-row items-center lg:items-start gap-6 lg:gap-8">
                {/* QR Code Image - Clean and centered on mobile */}
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-3 sm:p-4 shadow-lg shrink-0 w-full sm:w-auto flex justify-center">
                  <Image 
                    src={qrCodeUrl} 
                    alt="Event QR Code" 
                ...
                {/* Actions Column - Cleaner layout */}
                <div className="flex flex-col gap-4 flex-1 w-full">
                  {/* QR Code Actions */}
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                    <button 
                      onClick={copyQrToClipboard} 
                      className="flex-1 px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-blue-500/20 to-blue-600/20 hover:from-blue-500/30 hover:to-blue-600/30 border border-blue-400/30 rounded-xl text-white transition-all duration-200 flex items-center justify-center gap-2 font-medium backdrop-blur-sm text-sm sm:text-base"
                    >
                  ...
                  {/* Event Link Section - Cleaner with direct link button */}
                  <div className="pt-3 sm:pt-4 border-t border-white/10">
                    <h3 className="text-xs sm:text-sm font-normal text-white/80 mb-2 sm:mb-3 flex items-center gap-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <input
                        ...
                        value={guestLink}
                        readOnly
                        className="flex-1 px-3 sm:px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white/90 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                      />
                      <div className="flex gap-2">
                        <button
                          onClick={() => window.open(guestLink, '_blank')}
                          className="px-4 sm:px-5 py-2 bg-gradient-to-r from-emerald-500/20 to-emerald-600/20 hover:from-emerald-500/30 hover:to-emerald-600/30 border border-emerald-400/30 rounded-lg text-white transition-all duration-200 flex items-center justify-center gap-2 font-medium backdrop-blur-sm whitespace-nowrap text-xs sm:text-sm"
                        >
                          <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {/* Social Sharing Section - More compact on mobile */}
                  <div className="pt-3 sm:pt-4 border-t border-white/10">
                    <h3 className="text-xs sm:text-sm font-normal text-white/80 mb-2 sm:mb-3 flex items-center gap-2">
                      <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3">
                      ...
                      <button
                        ...
                        className="group relative px-2 sm:px-4 py-2 sm:py-3 bg-gradient-to-br from-[#0077b5]/20 to-[#0077b5]/30 hover:from-[#0077b5]/30 hover:to-[#0077b5]/40 border border-[#0077b5]/40 rounded-lg sm:rounded-xl transition-all duration-200 flex flex-col items-center gap-1 sm:gap-2 backdrop-blur-sm hover:scale-105 hover:shadow-lg hover:shadow-[#0077b5]/20"
                      >
                        <svg className="w-5 h-5 sm:w-6 sm:h-6 text-[#0077b5]" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                        </svg>
                        <span className="text-[10px] sm:text-xs font-medium text-white/90 hidden sm:inline">LinkedIn</span>
                      </button>

                      {/* X (Twitter) */}
                      <button
                        onClick={shareOnTwitter}
                        className="group relative px-2 sm:px-4 py-2 sm:py-3 bg-gradient-to-br from-black/40 to-gray-900/40 hover:from-black/50 hover:to-gray-900/50 border border-white/20 rounded-lg sm:rounded-xl transition-all duration-200 flex flex-col items-center gap-1 sm:gap-2 backdrop-blur-sm hover:scale-105 hover:shadow-lg hover:shadow-white/10"
                      >
                        <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                        </svg>
                        <span className="text-[10px] sm:text-xs font-medium text-white/90 hidden sm:inline">X.com</span>
                      </button>

                      {/* Facebook */}
                      <button
                        onClick={shareOnFacebook}
                        className="group relative px-2 sm:px-4 py-2 sm:py-3 bg-gradient-to-br from-[#1877f2]/20 to-[#1877f2]/30 hover:from-[#1877f2]/30 hover:to-[#1877f2]/40 border border-[#1877f2]/40 rounded-lg sm:rounded-xl transition-all duration-200 flex flex-col items-center gap-1 sm:gap-2 backdrop-blur-sm hover:scale-105 hover:shadow-lg hover:shadow-[#1877f2]/20"
                      >
                        ...
                        <span className="text-[10px] sm:text-xs font-medium text-white/90 hidden sm:inline">Facebook</span>
                      </button>

                      {/* WhatsApp */}
                      <button
                        onClick={shareOnWhatsApp}
                        className="group relative px-2 sm:px-4 py-2 sm:py-3 bg-gradient-to-br from-[#25d366]/20 to-[#25d366]/30 hover:from-[#25d366]/30 hover:to-[#25d366]/40 border border-[#25d366]/40 rounded-lg sm:rounded-xl transition-all duration-200 flex flex-col items-center gap-1 sm:gap-2 backdrop-blur-sm hover:scale-105 hover:shadow-lg hover:shadow-[#25d366]/20"
                      >
                        ...
                        <span className="text-[10px] sm:text-xs font-medium text-white/90 hidden sm:inline">WhatsApp</span>
                      </button>

                      {/* Instagram */}
                      <button
                        onClick={shareOnInstagram}
                        className="group relative px-2 sm:px-4 py-2 sm:py-3 bg-gradient-to-br from-[#E4405F]/20 via-[#F56040]/20 to-[#FCAF45]/20 hover:from-[#E4405F]/30 hover:via-[#F56040]/30 hover:to-[#FCAF45]/30 border border-[#E4405F]/40 rounded-lg sm:rounded-xl transition-all duration-200 flex flex-col items-center gap-1 sm:gap-2 backdrop-blur-sm hover:scale-105 hover:shadow-lg hover:shadow-[#E4405F]/20"
                      >
                        ...
                        <span className="text-[10px] sm:text-xs font-medium text-white/90 hidden sm:inline">Instagram</span>
                      </button>
                    </div>
                  </div>
                </div>
